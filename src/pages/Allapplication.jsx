  ...
  return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 20px", borderTop:`1px solid ${C.rowBorder}`, flexWrap:"wrap", gap:10 }}>
      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
        ...
        <span style={{ fontSize:12, color:C.muted, whiteSpace:"nowrap" }}>of <strong style={{ color:C.dark }}>{total}</strong></span>
      </div>
      {totalPages > 1 && (
        <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap" }}>
          <button onClick={()=>onPageChange(page-1)} disabled={page===1}
            style={{ ...btnBase, background:page===1?"#f0f4f0":C.white, color:page===1?"#aaa":C.green, cursor:page===1?"not-allowed":"pointer" }}>‹ Prev</button>
          {pages.map((p,i)=> p==="..." ?
            <span key={`dot-${i}`} style={{ padding:"6px 4px", fontSize:13, color:C.muted }}>…</span> :
            <button key={p} onClick={()=>onPageChange(p)}
              style={{ ...btnBase, background:p===page?C.green:C.white, color:p===page?"#fff":C.dark, border:`1.5px solid ${p===page?C.green:C.greenBorder}` }}>{p}</button>
          )}
          <button onClick={()=>onPageChange(page+1)} disabled={page===totalPages}
            style={{ ...btnBase, background:page===totalPages?"#f0f4f0":C.white, color:page===totalPages?"#aaa":C.green, cursor:page===totalPages?"not-allowed":"pointer" }}>Next ›</button>
        </div>
      )}
    </div>
  );
...
const [limit, setLimit]           = useState(20);
  ...
  const fetchApplications = async (pageNum = page, limitNum = limit) => {
    try {
      ...
      setTotal(res.data.total || 0);           // ← ADD
      setTotalPages(res.data.totalPages || 1); // ← ADD
      setPage(pageNum);    
      }                    // ← ADD
...
const handleLimitChange = (newLimit) => {
  ...
  const handleFormClose = () => {
    ...
    setPrefillData(null);
    // fetchApplications();
     fetchApplications(page, limit); // ← was fetchApplications()
  };
  ...
  const statusCounts = ["All","Pending","In Progress","Resolved","Rejected"].map(s => ({
    ...
    // count: s==="All" ? applications.length : applications.filter(a=>a.status===s).length,
      count: s==="All" ? total : applications.filter(a=>a.status===s).length, // ← total not applications.length
